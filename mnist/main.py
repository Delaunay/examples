...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        data, target = tools.enable_half(data.to(device)), tools.enable_half(target.to(device))
        optimizer.zero_grad()
        output = model(data)
...
def main():
    ...
    parser.add_argument('--data', type=str, default='../data', metavar='D',
                        help='location where the data is downloaded and read from')
    ...
    model = tools.enable_half(Net().to(device))

    optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=args.momentum)
    # half mode need a special optimizer for loss rescaling
    optimizer = tools.OptimizerAdapter(optimizer, half=tools.use_half(), static_loss_scale=args.static_loss_scale)
    # static_loss_scale=16384.0 / 2
    # dynamic_loss_scale=True, dynamic_loss_args={'init_scale': 128, 'scale_window': 5000})
    ...
    end_time = time.time()
    print('Trained in {:4.2f} s'.format(end_time - start_time))
